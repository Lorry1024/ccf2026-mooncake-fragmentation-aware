...
\begin{titlepage}
  ...
  \begin{tabular}{|>{\sffamily\bfseries\centering\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{9.3cm}|}
    ...
    队伍名称 & KVCache Forge \\ \hline
    队伍成员 & 罗荣裕（队长）、刘神舟、王英颖、杨俊 \\ \hline
    GitHub仓库 & \url{https://github.com/Lorry1024/ccf2026-mooncake-fragmentation-aware} \\ \hline
    GitLink仓库 & \url{https://gitlink.org.cn/lorry22/ccf2026-mooncake-fragmentation-aware} \\ \hline
...
\begin{table}[h]
  ...
  \begin{tabularx}{\textwidth}{|B{3.2cm}|Y|}
    ...
    展示视频 & 5分钟作品展示视频已录制并上传，链接与提取码见\artifact{README.md}和\artifact{SUBMISSION.md}。 \\ \hline
    后续工作 & 真实RDMA和SGLang HiCache端到端benchmark。 \\ \hline
  \end{tabularx}
\end{table}
...
图\ref{fig:pr-overview}给出本项目在Mooncake上游仓库中的公开PR页面。截图同时展示了PR编号2797、draft状态、目标分支\artifact{kvcache-ai:main}、来源分支\path{Lorry1024:ccf-fragmentation-aware-allocation}、4次提交和9个改动文件，使代码来源、上游目标和评审入口能够被直接核验。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{pr_2797_overview.png}
  \caption{Mooncake上游PR\#2797总览}
...
\begin{lstlisting}[language=C++,caption={候选碎片元数据与请求相关评分的实际实现}]
...
const double score =
    0.70 * contiguity_ratio + 0.30 * free_ratio;
\end{lstlisting}

第一段代码是本项目相对\baseline{}的关键差异。原策略主要使用\artifact{free\_ratio}描述“还剩多少空间”，新策略进一步读取allocator报告的最大连续空闲区域，并通过\artifact{can\_fit}把请求大小\artifact{slice\_length}直接引入决策。若某类allocator暂时无法提供精确的连续空闲信息，代码使用当前总空闲量作为保守退化值；若能够提供，则用\artifact{min(reported\_largest, free)}限制异常元数据，避免最大连续区超过实际总空闲量。连续性比例反映空闲空间是否集中，0.70与0.30的权重使同为可容纳候选时优先保护连续空间，同时保留一定空间均衡能力。

\begin{lstlisting}[language=C++,caption={候选排序优先级的实际实现}]
...
第二段代码给出完整排序优先级。\artifact{can\_fit}是最高优先级，从根本上阻止“总空闲较高但当前对象无法落地”的候选排在可容纳候选之前；综合score只在相同可容纳状态内比较；最大连续区域与总空闲比例继续作为细粒度tie-break；最后使用\artifact{name\_idx}形成确定性的稳定顺序，避免完全相同元数据下出现不可解释抖动。排序只改变尝试顺序，新策略没有接管底层空间管理，也没有绕过\artifact{allocateSingle}路径。preferred阶段、有界采样、excluded/used过滤、实际allocator申请和best-effort fallback均保留，因此评分元数据即使在并发下发生变化，也只会导致某次尝试失败并继续后续候选，不会突破空间安全边界。
...
图\ref{fig:ci-all-checks}展示PR合并检查区域：26项检查成功，1项按条件跳过，页面明确给出“All checks have passed”。跳过项是按触发条件不执行的自动检查，不表示构建或测试失败。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{ci_all_checks_passed.png}
  \caption{PR\#2797全部必需检查通过}
  \label{fig:ci-all-checks}
\end{figure}

图\ref{fig:ci-build-jobs}进一步展示Linux workflow中的构建矩阵。Python 3.10/3.12构建、ARM64构建、不同编译标志、Docker镜像、wheel以及CI Gate等任务均显示为成功，说明改动没有局限在单一编译组合中。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{ci_build_jobs.png}
  \caption{GitHub Actions Linux构建与多配置任务成功}
  \label{fig:ci-build-jobs}
\end{figure}

图\ref{fig:ci-full-test}是\artifact{build (3.10)}任务中的测试汇总，页面显示90项测试全部通过、0项失败。该结果与定向碎片化测试共同构成“目标行为正确”和“既有功能未回归”两类证据。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.94\textwidth]{ci_full_test_passed.png}
  \caption{build (3.10)环境下90项回归测试全部通过}
  \label{fig:ci-full-test}
\end{figure}
...
开源规范性则通过公开材料仓库、Mooncake fork分支和上游PR共同体现。KVCache Forge队伍在GitHub和GitLink均设置了公开比赛材料入口：GitHub仓库用于持续维护中文报告、PPT、复现程序、日志和提交压缩包，GitLink仓库用于赛事平台关联与队伍材料展示；Mooncake fork保留完整开发分支；上游PR\#2797展示代码差异、提交历史和GitHub Actions结果。这些入口分别回答“成果如何阅读”“赛事材料从何访问”“代码如何复核”和“上游工程是否接受验证”等问题。相比只提交一个patch文件，该组织方式更便于评审者追踪来源、核对许可证并复现关键结论。
...
\thfirst{文件或目录} & \thcell{说明} \\ \hline
\endhead
  \artifact{README.md} & 项目入口、快速使用和成果索引 \\ \hline
  \artifact{SUBMISSION.md} & 平台提交说明与必交材料对应关系 \\ \hline
  \artifact{DESIGN.md} & 问题、目标、算法与兼容性设计 \\ \hline
  \artifact{EVALUATION.md} & CI、仿真指标和结论边界 \\ \hline
  \artifact{demo\_script.md} & 5分钟作品展示逐字稿与分镜 \\ \hline
  \splitpath{mooncake\_fragmentation\_aware}{pr\_2797\_0123fa1.patch} & 当前PR补丁 \\ \hline
  \artifact{repro/} & 可独立编译的复现实验源码 \\ \hline
  \artifact{logs/} & 三份关键仿真结果日志 \\ \hline
  \splitpath{tools}{run\_powershell\_demo.ps1} & Windows PowerShell一键演示脚本 \\ \hline
  \artifact{slides/} & 初赛展示PPT最终版 \\ \hline
  \artifact{report/} & 本技术报告源码、PDF和证据截图 \\ \hline
...
\begin{itemize}
  ...
  \item GitHub比赛材料仓库：\href{https://github.com/Lorry1024/ccf2026-mooncake-fragmentation-aware}{Lorry1024/ccf2026-mooncake-fragmentation-aware}
  \item GitLink比赛材料仓库：\href{https://gitlink.org.cn/lorry22/ccf2026-mooncake-fragmentation-aware}{lorry22/ccf2026-mooncake-fragmentation-aware}
  ...
  \item 作品展示视频：\href{https://pan.baidu.com/s/1v2-jxHSaDi0RlagPGrLAiw?pwd=mxhx}{百度网盘，提取码mxhx}
\end{itemize}
