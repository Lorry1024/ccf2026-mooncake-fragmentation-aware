...
从工程路径看，完整工作被拆成六步。第一步，阅读Mooncake Store已有分配策略，确认\artifact{random}、\baseline{}等策略的职责边界。第二步，构造最小碎片化反例，证明总空闲比例和连续可用空间之间存在可观察的不一致。第三步，设计\strategy{}评分字段，把最大连续空闲区域、连续性比例和当前请求大小引入候选排序。第四步，把策略接入Mooncake Store正式配置路径，而不是停留在离线脚本或实验代码中。第五步，补充单元测试、benchmark矩阵和文档说明，保证策略可使用、可回滚、可被评审。第六步，通过上游PR和GitHub Actions完成开源项目层面的基本验证。

\begin{table}[htbp]
  \centering
  \caption{从赛题要求到本项目技术路线的分解}
  \label{tab:route-overview}
  \begin{tabularx}{\textwidth}{|B{3.2cm}|L{4.1cm}|Y|}
    \hline
    层次 & 具体问题 & 本项目处理方式 \\ \hline
    赛题目标 & 提升Mooncake Store吞吐、可扩展性和HiCache后端表现 & 选择Store内部高频放置决策作为优化入口 \\ \hline
    系统现象 & 混合大小对象使segment内部出现碎片 & 以最大连续空闲区域刻画碎片化对当前请求的影响 \\ \hline
    原有不足 & \baseline{}只根据总空闲比例排序 & 增加\artifact{can\_fit}和连续性评分，使排序更贴近真实可分配性 \\ \hline
    工程约束 & 不能破坏默认行为、协议和副本语义 & 作为显式可选策略接入，保留preferred、excluded和fallback路径 \\ \hline
    验证方式 & 初赛无法完全覆盖真实RDMA和SGLang集群 & 先用单元测试、专题仿真、benchmark接入和上游CI证明策略正确性 \\ \hline
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{KVCache Store中碎片形成的典型原因}
  \label{tab:fragment-sources}
  \begin{tabularx}{\textwidth}{|B{3.2cm}|L{4.5cm}|Y|}
    \hline
    来源 & 具体表现 & 对分配策略的影响 \\ \hline
    对象大小混合 & 不同请求产生的KVCache对象长度差异较大 & 大对象更依赖连续空闲区域，小对象更容易填入零散空洞 \\ \hline
    生命周期不同步 & 部分缓存被提前淘汰，部分缓存长期保留 & 空闲区间分散，segment内部形态随时间恶化 \\ \hline
    多副本写入 & 同一对象可能需要放入多个segment & 一个副本的错误首选会放大整体fallback次数 \\ \hline
    前缀复用与淘汰 & 复用命中和淘汰策略改变对象保留时间 & 同批写入对象不一定同批释放，空间回收不规则 \\ \hline
    长时间运行 & 分配和释放持续交替发生 & 总空闲比例逐渐不能代表当前大对象可分配性 \\ \hline
  \end{tabularx}
\end{table}
...
首先，fallback通常意味着前一次候选已经做过元数据读取、排除集合判断、allocator尝试和失败处理。其次，多副本请求中每个副本都要避免重复使用同一segment，因此一个副本的失败会改变后续副本的候选空间。再次，fallback路径本质上是补救逻辑，它可以提高可用性，但不应成为碎片化场景下的常态路径。更理想的做法是在排序阶段就把明显不可容纳的候选排到后面，让系统优先尝试成功概率更高的segment。

\begin{table}[htbp]
  \centering
  \caption{依赖fallback与碎片感知排序的差异}
  \label{tab:fallback-vs-aware}
  \begin{tabularx}{\textwidth}{|B{3.1cm}|L{4.9cm}|Y|}
    \hline
    比较项 & 依赖fallback & 碎片感知排序 \\ \hline
    决策位置 & allocator失败后补救 & allocator尝试前优化候选顺序 \\ \hline
    首选质量 & 可能被总空闲比例误导 & 优先判断最大连续空闲区域是否足够 \\ \hline
    多副本影响 & 每个副本都可能经历额外失败尝试 & 尽量让每个副本从高质量候选开始 \\ \hline
    延迟稳定性 & 失败次数随碎片状态波动 & 减少明显无效候选带来的尾部波动 \\ \hline
    工程语义 & 保底路径，不能删除 & 主路径排序优化，可与fallback共存 \\ \hline
  \end{tabularx}
\end{table}
...
本项目在设计中保留三个不变量。第一，任何被excluded的segment都不能被选中。第二，同一次多副本分配中，已经成功使用的segment会进入used集合，后续副本不能重复选择。第三，preferred segment具有显式优先级，只要preferred可用并能完成分配，就不进入后续评分排序。这些不变量保证新策略不会改变调用方已经依赖的放置语义。

\begin{table}[htbp]
  \centering
  \caption{\strategy{}的输入、输出与保持的不变量}
  \label{tab:strategy-io}
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.5cm}|Y|}
    \hline
    类型 & 内容 & 约束说明 \\ \hline
    输入 & 请求大小\artifact{slice\_length} & 用于判断最大连续空闲区域是否能够容纳当前对象 \\ \hline
    输入 & 副本数\artifact{replica\_num} & 决定需要返回多少个成功分配结果 \\ \hline
    输入 & preferred segments & 先于采样候选尝试，保留上层显式调度语义 \\ \hline
    输入 & excluded segments & 在preferred、采样和fallback阶段均不得选择 \\ \hline
    内部状态 & used segments & 避免同一次多副本请求重复使用同一segment \\ \hline
    输出 & 成功申请的buffer列表 & 每个结果都来自allocator实际分配，而不是评分阶段的预测 \\ \hline
  \end{tabularx}
\end{table}
...
更重要的是，\strategy{}并不直接用score替代所有排序条件。它先比较\artifact{can\_fit}，再比较score。这意味着当候选A无法容纳当前请求、候选B可以容纳当前请求时，即使A的总空闲比例更高，也不会排在B前面。这个设计避免了“高分但不可用”的情况。score只在同一fit状态内部发挥作用，用于区分多个都可用或多个都不可用的候选。

\begin{table}[htbp]
  \centering
  \caption{不同排序信号的取舍}
  \label{tab:signal-tradeoff}
  \begin{tabularx}{\textwidth}{|B{3.3cm}|L{4.4cm}|Y|}
    \hline
    排序方式 & 优点 & 缺点 \\ \hline
    只看总空闲比例 & 有利于segment整体空间均衡 & 无法判断当前请求是否存在足够连续空间 \\ \hline
    只看最大连续空闲区域 & 对大对象分配直接有效 & 可能牺牲整体空间均衡，过度偏向少数segment \\ \hline
    只看连续性比例 & 能识别空闲空间是否集中 & 可能忽略绝对空闲容量，较小segment也可能得分高 \\ \hline
    先看\artifact{can\_fit}再看综合score & 当前可分配性优先，同时保留空间均衡信号 & 比\baseline{}多一次碎片元数据读取和排序比较 \\ \hline
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|r|r|r|l|Y|}
    \hline
    segment & 总空闲 & 最大连续空闲 & 请求大小 & \artifact{can\_fit} & 排序解释 \\ \hline
...
这种处理方式也解释了为什么策略没有输出一次性全局排序计划。Store中的空间状态可能在并发请求之间快速变化，提前构造完整计划并不能保证全部成功。更稳妥的方式是：每个副本在当前状态下选择高质量候选，成功后立即更新used集合；失败则继续尝试候选或fallback。这样可以在不引入复杂全局锁和事务计划的情况下，保持较好的工程鲁棒性。

\begin{table}[htbp]
  \centering
  \caption{多副本分配中的状态变化}
  \label{tab:replica-state}
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.4cm}|Y|}
    \hline
    阶段 & 动作 & 目的 \\ \hline
    副本开始 & 读取excluded和used集合 & 排除不可选和已使用segment \\ \hline
    preferred尝试 & 优先尝试用户指定位置 & 保留上层调度语义 \\ \hline
    候选排序 & 对未排除、未使用候选进行碎片感知排序 & 提高当前副本首选成功率 \\ \hline
    分配成功 & 将segment加入used集合 & 避免后续副本重复选择同一位置 \\ \hline
    分配不足 & 进入原有fallback路径 & 保证best-effort语义不被削弱 \\ \hline
  \end{tabularx}
\end{table}
...
这段逻辑体现了一个重要原则：新策略并没有接管底层空间管理，也没有绕过已有\artifact{allocateSingle}路径。它只在调用\artifact{allocateSingle}之前调整候选顺序。这样做能最大限度降低行为风险，因为空间是否真的可分配仍由原有allocator确认；同时，新策略又能在排序阶段规避最明显的碎片化误判。

\begin{table}[htbp]
  \centering
  \caption{实现阶段与代码职责}
  \label{tab:implementation-phases}
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.3cm}|Y|}
    \hline
    阶段 & 代码职责 & 设计意图 \\ \hline
    preferred阶段 & 遍历preferred segment并尝试分配 & 保持调用方显式放置优先级 \\ \hline
    采样阶段 & 从可用segment中得到候选集合 & 保持策略开销有界，避免全局扫描 \\ \hline
    评分阶段 & 收集总空闲、容量和最大连续空闲区域 & 把碎片信息转化为排序字段 \\ \hline
    排序阶段 & 按\artifact{can\_fit}、score和tie-break排序 & 让可容纳当前请求的候选优先 \\ \hline
    分配阶段 & 调用原有allocator申请空间 & 保证最终正确性仍由allocator判定 \\ \hline
    fallback阶段 & 未满足副本时走原有兜底逻辑 & 保留best-effort副本语义 \\ \hline
  \end{tabularx}
\end{table}
...
灰度使用时，可以先在测试环境或单个Store实例中启用新策略，观察失败分配次数、fallback次数、写入延迟和空间利用率。如果指标改善，再扩大到更多实例；如果发现某类负载下新策略收益不足或元数据读取成本偏高，只需要调整启动参数即可回滚，不需要回滚二进制或修改客户端。

\begin{table}[htbp]
  \centering
  \caption{策略启用和回滚方式}
  \label{tab:rollout}
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.8cm}|Y|}
    \hline
    场景 & 配置 & 说明 \\ \hline
    默认行为 & 不传或使用\artifact{random} & 完全保留Mooncake Store现有默认策略 \\ \hline
    空间均衡基线 & \splitpath{--allocation\_strategy}{=free\_ratio\_first} & 使用总空闲比例优先策略，作为对照基线 \\ \hline
    碎片优化 & \splitpath{--allocation\_strategy}{=fragmentation\_aware} & 启用本项目新增策略，适合混合大小对象和长期运行场景 \\ \hline
    快速回滚 & 改回\artifact{random}或\baseline{} & 不需要修改客户端协议，不需要迁移已有数据 \\ \hline
  \end{tabularx}
\end{table}
...
碎片化测试的构造要避免一个常见陷阱：如果释放的是靠近尾部的块，allocator可能把释放块和尾部剩余空间合并，导致最大连续空闲区域大于预期。这样测试看似构造了碎片，实际却没有形成“大量空闲但连续块不足”的状态。最终PR中的测试选择释放内部块，确保\artifact{fragmented}的总空闲空间较高，但最大连续空闲区域仍小于请求大小；同时构造\artifact{contiguous}，使其总空闲空间较低但最大连续区域满足请求。这种测试更接近问题定义，能稳定验证排序信号是否正确。

\begin{table}[htbp]
  \centering
  \caption{碎片化单元测试的构造逻辑}
  \label{tab:unit-construction}
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.5cm}|Y|}
    \hline
    步骤 & 构造动作 & 验证意义 \\ \hline
    构造fragmented & 分配多个固定大小块，再释放内部块 & 形成总空闲较高但连续空闲不足的segment \\ \hline
    构造contiguous & 保留一段足够大的连续空闲区域 & 提供能够满足当前请求的对照segment \\ \hline
    设置请求大小 & 请求大于fragmented最大连续块，小于contiguous最大连续块 & 强制暴露总空闲比例误判问题 \\ \hline
    执行新策略 & 调用\strategy{}进行分配 & 验证策略优先选择可容纳请求的segment \\ \hline
    检查preferred & 单独传入preferred segment & 验证兼容性语义没有被评分逻辑破坏 \\ \hline
  \end{tabularx}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|B{3.0cm}|L{4.5cm}|Y|}
    \hline
    验证层次 & 主要材料 & 回答的问题 \\ \hline
    代码级验证 & 单元测试、参数化测试、上游CI & 新策略是否能安全接入Mooncake代码路径 \\ \hline
    问题级验证 & 碎片化定向测试 & 新策略是否真的避免总空闲比例误判 \\ \hline
    指标级验证 & 专题仿真和日志 & 首选成功率、fallback次数和决策开销如何变化 \\ \hline
    工程级验证 & 文档、benchmark接入和PR流程 & 后续评审者是否能复现、理解并继续扩展 \\ \hline
  \end{tabularx}
\end{table}
