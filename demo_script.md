...
建议在WSL或Ubuntu环境中提前完成编译，录制时只运行程序，避免把等待编译的时间录进视频：
...
如果还要展示决策耗时，可提前编译第二个程序：

```bash
g++ -std=c++17 -O2 repro/fragmentation_aware_metrics.cpp \
  -o fragmentation_aware_metrics
./fragmentation_aware_metrics
```

录制前清空终端，只保留简短提示符。运行结果必须与仓库参考日志一致，至少能看到0/6、6/6、11到0和平均候选数量5.00。
...
**画面：**切换到终端，输入并执行以下命令。命令可以提前放在历史记录中，用方向键调出，避免打错。
